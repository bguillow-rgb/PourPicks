...
- A straight bourbon **under four years old must show an age statement.** No exceptions.
- A bourbon **four years or older can legally omit the age.** This is why so many bottles carry no age at all, the distiller has decided the number does not help them sell it.
...
- **Single barrel** means the bottle was filled from one individual barrel. Because no two barrels age identically, each batch (and sometimes each bottle run) varies a little. Store picks are almost always single barrels.
- **Small batch** means the whiskey was blended from a limited number of barrels to achieve a consistent profile. There is **no legal definition** of "small," so the actual barrel count ranges from a handful to many dozens depending on the brand. Treat it as a style cue, not a guarantee.
- **Straight** is the regulated one. A **straight bourbon** has been aged at least two years and contains **no added coloring, flavoring, or other spirits.** If it is under four years old, it must also state its age.

The clean way to remember it: single barrel and small batch describe **selection** (how the barrels were chosen and combined), while straight describes **purity** (what is and is not allowed in the bottle). For more on how the grain recipe behind all of this shapes flavor, see [what is a mash bill](/articles/what-is-mash-bill), and for how bourbon differs from its closest cousin, [bourbon vs rye](/articles/bourbon-vs-rye-difference).

## A quick tour of the rest of the label

A few more terms you will run into:

- **"Kentucky Straight Bourbon"** adds a geographic guarantee: it was distilled and aged in Kentucky. Bourbon can legally be made anywhere in the US, so the state line is a real, if minor, distinction.
- **"Sour mash"** means a portion of a previous fermentation was used to start the next, a near-universal practice in bourbon. It sounds special but tells you almost nothing, because nearly everyone does it.
- **"Distilled by" vs "Produced by" vs "Bottled by"** can reveal whether the brand actually distilled the whiskey or sourced it from another distillery. "Distilled by" is the strongest claim.
- **Batch and bottle numbers** (often hand-written on limited releases) matter to collectors for provenance and resale, and they are exactly the kind of detail a barcode cannot capture.
